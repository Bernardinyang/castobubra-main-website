                                ...
                                <div class="toggle-expand-content" data-content="pageMenu">
                                    <ul class="nk-block-tools g-3">
                                        <li class="nk-block-tools-opt">
                                            <a href="{{ route('user.contacts') }}" class="btn btn-secondary">
                                        ...
                                        @if(auth()->user()->hasRole('developer'))
                                            <li class="nk-block-tools-opt">
                                                <form class="btn btn-danger" onclick="this.submit()"
                                                      action="{{ route('user.contact.destroy', $contact->unique_id) }}"
                                                      method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        @endif
                                        <li>
                                            <form class="btn btn-secondary" onclick="this.submit()"
                                                  action="{{ route('user.contact.reply', $contact->unique_id) }}"
                                                  method="POST">
                                                @csrf
                    ...
                    @if(session('status_type') && session('status_message'))
                        <div class="alert alert-pro alert-{{ session('status_type') }}">
                            <div class="alert-text">
                                <code>{{ session('status_message') }}</code>
                            </div>
                        </div>
                    @endif
                    <div class="container">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" readonly disabled
                                               class="form-control form-control-lg"
                                               id="time"
                                               value="{{ \App\Http\Services\DateHelperService::formatDate($contact->created_at) }} ( {{ \App\Http\Services\DateHelperService::toHuman($contact->created_at) }} )">
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-6 mb-3">
